...
(function(global) {
  ...
  var augment_new = function(V, crop_w, crop_h, dx, dy, fliplr) {
    // note assumes square outputs of size crop x crop
    if(typeof(fliplr)==='undefined') var fliplr = false;
    if(typeof(dx)==='undefined') var dx = global.randi(0, V.sx - crop_w);
    if(typeof(dy)==='undefined') var dy = global.randi(0, V.sy - crop_h);
    
    // randomly sample a crop in the input volume
    var W;
    if(crop_w !== V.sx || crop_h!== V.sy || dx!==0 || dy!==0) {
      W = new Vol(crop_w, crop_h, V.depth, 0.0);
