var sample_training_instance = function() {
  ...
  var x = new convnetjs.Vol(image_dimension_w,image_dimension_h,image_channels,0.0);
  var W = image_dimension_h*image_dimension_w;
  var j=0;
  for(var dc=0;dc<image_channels;dc++) {
    var i=0;
    for(var xc=0;xc<image_dimension_h;xc++) {
      for(var yc=0;yc<image_dimension_w;yc++) {
        var ix = ((W * k) + i) * 4 + dc;
  ...
  if(random_position){
    ...
    x = convnetjs.augment_new(x, image_dimension_w, image_dimension_h, dx, dy, false); //maybe change position
  }

  if(random_flip){
    x = convnetjs.augment_new(x, image_dimension_w, image_dimension_h, 0, 0, Math.random()<0.5); //maybe flip horizontally
  }
...
var sample_test_instance = function() {
  ...
  var p = img_data[b].data;
  var x = new convnetjs.Vol(image_dimension_w,image_dimension_h,image_channels,0.0);
  var W = image_dimension_w*image_dimension_h;
  var j=0;
  for(var dc=0;dc<image_channels;dc++) {
    var i=0;
    for(var xc=0;xc<image_dimension_h;xc++) {
  ...
  if (random_flip || random_position){
    for(var k=0;k<6;k++) {
      ...
      if(random_position){
        var dx = Math.floor(Math.random()*5-2);
        var dy = Math.floor(Math.random()*5-2);
        test_variation = convnetjs.augment_new(test_variation,image_dimension_w, image_dimension_h, dx, dy, false);
      }
      
      if(random_flip){
        test_variation = convnetjs.augment_new(test_variation,image_dimension_w, image_dimension_h, 0, 0, Math.random()<0.5); 
      }

      xs.push(test_variation);
    }
  }else{
    xs.push(x, image_dimension_w, image_dimension_h, 0, 0, false); // push an un-augmented copy
  }
...
var load_data_batch = function(batch_num) {
  ...
  data_img_elt.onload = function() { 
    ...
    console.log(data_img_elt.width, data_img_elt.height);
...
var test_predict = function() {
  ...
  for(num=0;num<10;num++) {
    ...
    var pad_top = image_dimension_h/2 - image_dimension_h/4;
