...
var start_time;
var end_time;
var testMulti_Image = function(images_list, imagelabels){
  var correct = 0;
  var inCorrect = 0;
  var total = 0;
  var OneImgacc = 0;
  ...
  for(var i=0; i<images_list.length; i++){
      var y = imagelabels[i];
      var x = convnetjs.img_to_vol(images_list[i]);
      ...
      draw_activations_COLOR(div, x, 1);
      ...
      for(var k=0;k<2;k++) {
        ...
        t += '<div class=\"pp\" style=\"width:' + parseFloat(preds[k].p/1*100).toFixed(2) + 'px; margin-left: 10px; background-color:' + col + ';\">' + classes_txt[preds[k].k] + '</div>'
      }
      t+= '</div>';
      total++;
      console.log(total);
      ...
      $(div).prependTo($("#testset-vis-folder")).hide().fadeIn(100).slideDown(100);
  }

  d = new Date();
  end_time = d.getTime();
  var time = end_time - start_time;
  var testAccvalue = parseFloat((correct/total)*100).toFixed(2);
  $('#sumImageFolder').text('- The number of images were classified: ' + total);
  $("#testset-acc-total").text('- Test accuracy: ' + testAccvalue + ' %');
  $("#testset-acc-correct").text('- The number of images were classified correctly: ' + correct);
  $("#testset-acc-incorrect").text('- The number of images were classified incorrectly: ' + inCorrect);
  $("#testset-acc-time").text('- Inference time: '+ time + ' ms');
}
...
//Load pretrained
var load_network_pretrained = function(file_url) {
    var f = file_url;
...
$(function(){
  $('#imgInp').change(function(){
      img_label = getImageLabelFromInput(this);
      img_list = getImageFromInput(this);
      $('#sumImageFolder').text("Load image successful!");

      $("#testset-acc-total").text('');
      $("#testset-acc-correct").text('');
