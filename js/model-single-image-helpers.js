...
function resize(src){
  ...
  return new Promise(function (resolve) {
    image.onload = function () {
      ...
      window.pica.WW = true;
      resolve(dst.toDataURL("image/jpg"));
    };
  });
}
...
$(document).ready( function() {
        ...
        function readURL(input) {
            if (input.files && input.files[0]) {
                ...
                img_preview_128_hide.className = "img-preview";
                img_preview_128_hide.id = "preview_img_128";
                img_preview_128_hide.setAttribute("style","width:"+image_dimension_w+"px");
                img_preview_128_hide.setAttribute("style","height:"+image_dimension_h+"px");

                reader.onload = function () {
                var preview = document.getElementById('preview_img_128');
                var preview_show = document.getElementById('preview_img_128_show');
                centerCrop(reader.result)
                    .then(resize)
                    .then(function (src) {
                        preview.src = src;
